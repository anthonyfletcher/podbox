...
| Directory | Rule |
|---|---|
| `draw/` | Stateless drawing. Knows pixels; knows nothing about features. viewport, line, scrollbar, icon, bmp, resize, jpeg decode, screen_access. |
...
| `viewers/` | File-format applications: their own model, view and input. `text_viewer/` (streaming document engine), `image_viewer/`, properties, playing_time, credits. |
| `skin/` | The skin interpreter, backdrops, and the skinned status bar. |

`screens/` is subdivided:

- `browse/` — the browser and its two backends (disk, db)
- `covers/` — cover flow: album covers, artist portraits
- `playback/` — wps, track info, quick screen, pitch screen
- `settings/` — the settings screens, every `*_settings` file
- `system/` — debug menu, log viewer, usb, runtime info, time set, alarm

and at the top level: bookmark, context_menu, shortcuts, main_menu.
