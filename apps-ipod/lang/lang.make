...
# $(APPSBUILDDIR)/lang must exist before we create dependencies on it,
# otherwise make will simply ignore those dependencies.
# Therefore we create it here.
#DUMMY := $(shell mkdir -p $(APPSBUILDDIR)/lang)
...
$(BUILDDIR)/lang/max_language_size.h: $(LANGOBJ) $(APPSBUILDDIR)/lang/voicestrings.zip
	$(call PRINTS,GEN $(subst $(BUILDDIR)/,,$@))
	$(SILENT)echo "#define MAX_LANGUAGE_SIZE `ls -ln $(APPSBUILDDIR)/lang/*.lng | awk '{print $$5-10}' | sort -n | tail -1`" > $@
...
$(BUILDDIR)/lang/lang.h: $(APPSDIR)/lang/$(ENGLISH).lang $(APPSBUILDDIR)/genlang-features $(TOOLSDIR)/genlang
	$(call PRINTS,GEN lang.h)
	$(SILENT)$(TOOLSDIR)/genlang -e=$(APPSDIR)/lang/$(ENGLISH).lang -p=$(BUILDDIR)/lang -t=$(MODELNAME):`cat $(APPSBUILDDIR)/genlang-features` $<
...
$(BUILDDIR)/%.lng $(BUILDDIR)/%.vstrings: $(ROOTDIR)/%.lang $(APPSDIR)/lang/$(ENGLISH).lang $(APPSBUILDDIR)/genlang-features $(TOOLSDIR)/genlang $(TOOLSDIR)/updatelang
	$(call PRINTS,GENLANG $(subst $(ROOTDIR)/,,$<))
	$(SILENT)mkdir -p $(dir $@)
	$(SILENT)$(TOOLSDIR)/updatelang $(APPSDIR)/lang/$(ENGLISH).lang $< $@.tmp
	$(SILENT)$(TOOLSDIR)/genlang -e=$(APPSDIR)/lang/$(ENGLISH).lang -t=$(MODELNAME):`cat $(APPSBUILDDIR)/genlang-features` -i=$(TARGET_ID) -b=$*.lng -c=$*.vstrings $@.tmp
	$(SILENT)rm -f $@.tmp

$(APPSBUILDDIR)/lang/voice-corrections.txt: $(ROOTDIR)/tools/voice-corrections.txt
	$(SILENT)mkdir -p $(dir $@)
	$(call PRINTS,CP $(subst $(ROOTDIR)/,,$<))cp $< $@

$(APPSBUILDDIR)/lang/lang-enum.txt: $(BUILDDIR)/lang_enum.h
	$(SILENT)mkdir -p $(dir $@)
...
$(APPSBUILDDIR)/lang/%.talk: $(COREAPPSDIR)/lang/%.talk
	$(call PRINTS,CP $(subst $(ROOTDIR)/,,$<))cp $< $(APPSBUILDDIR)/lang
