...
/* What a cell's surfaces would be with the switch in that state. The three
 * switch flags are three answers to the same question and are read here and
 * nowhere else: SP takes the platforms away until the switch is thrown, SF
 * takes the floor with them, and SX trades one for the other -- the
 * platforms stand over a hole, and throwing the switch fills the hole and
 * takes the platforms. */
static unsigned int spk_live_with(unsigned int e, bool on)
{
    ...
    if (e & SPK_C_SXF)
        return on ? (mask & 1u) : (mask & ~1u);

    if ((e & SPK_C_SPF) && !on)
        return (e & SPK_C_SFF) ? 0u : (mask & 1u);
...
{
    return (spk_gen_cell(cell) & (SPK_C_SPF | SPK_C_SXF)) != 0;
}
...
enum spk_outcome spk_state_peek(const struct spk_state *st)
{
    ...
    /* A step that does not come down to reach a creature walks into it. A
     * jump is never this case: it lands from above whatever levels it
     * spans, which is what makes a stomp a thing you go and do. */
    if (st->motion == SPK_WALK && st->from <= st->to)
    {
        int cl;
...
enum spk_outcome spk_state_advance(struct spk_state *st)
{
    ...
    case SPK_ARC_RISE:
         ...
         * that is what passing over means. A diamond at the top of the arc
         * is still collected, which is the whole of why the high ones are
         * worth a press. */
        st->level = -1;
        st->motion = SPK_ARC_FALL;
        out = spk_state_arrive(st, st->from + 2, false, true);
        break;
...
bool spk_state_jump(struct spk_state *st)
{
    ...
    st->from = st->level;
    st->to = spk_world_surface(st->beat + 2, st->level + 2);
