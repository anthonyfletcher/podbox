...
/* Single mode: this track change is to be followed by a pause. Decided when
 * the change is scheduled, acted on when it completes. */
...
static bool halt_decoding_track(bool stop)
{
    ...
    codec_seeking = false;
    /* whatever track change was pending is not happening now */
                                ...
                                bool is_current_track, int i, bool artist)
{
    ...
    if (wps_size_idx == -2)
        wps_size_idx = art_cache_size_index("wps");
    if (wps_size_idx < 0)
        return ERR_UNSUPPORTED_TYPE;

    sep = strrchr(track_id3->path, '/');
    if (!sep)
        return ERR_UNSUPPORTED_TYPE;
    dlen = sep - track_id3->path;
    ...
    return load_album_art_from_path(path, user_data, is_current_track, i);
}
...
static bool single_mode_do_pause(int id3_hid)
{
    if (global_settings.single_mode != SINGLE_MODE_OFF && global_settings.party_mode == 0 &&
        ...
        /* Playlist mode pauses only at a playlist boundary. The tag
         * comparison below cannot decide this one -- there is no
         * SINGLE_MODE_PLAYLIST tag to compare -- so answer here either way. */
...
static void audio_finalise_track_change(void)
{
    ...
    if (have_info)
    {
        ...
        /* Recomputed as well as remembered: when pause_on_track_change was
         * decided the next track_info may have been missing, on a low buffer
         * or an automatic directory change. */
 ...
 */
    ...
    if (old_status != PLAY_STOPPED)
    {
        ...
        /* Restarting, or resuming from pause, leaves audio in the PCM buffer
         * that crossfade would otherwise mix into whatever plays next --
         * audible as the old track bleeding into a manually chosen new one. */
