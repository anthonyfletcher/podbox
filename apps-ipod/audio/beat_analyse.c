 ...
 *
 * Reads decoded audio the DAC has not reached yet and turns it into onsets.
 *
 ...
 * is heard; everything here exists to turn those samples into timestamped
 * events without touching playback.
 *
 * Parts, in order:
 *   - tuning constants and module state
 *   - the cursor: seeding it, and what invalidates it
 *   - per-hop analysis: band magnitudes, spectral flux, onset decision
 *   - the poll loop that feeds hops from PCM chunks
...
static unsigned long base_ms;       /* Track time of the last stamped chunk */
static unsigned long base_frames;   /* Frames analysed since that stamp */
static bool          base_stamped;  /* A chunk has said where in the track
                                       the cursor is, so base_ms means
                                       something */
...
/* Drop everything that assumes samples run continuously. The running means
 * survive: they describe the mix rather than the position in it, and
 * rebuilding them from nothing costs a burst of false onsets. */
static void beat_resync(void)
{
    retain_fill = 0;
    have_mag_prev = false;
...
/* Fold one chunk's frames into the retained samples, analysing each time a
 * hop's worth of new ones has arrived. */
static void beat_feed(const struct pcmbuf_peek *peek)
{
    int i = 0;

    /* Hops fall on multiples of BEAT_HOP counted from the track's own
     * zero, not from wherever the cursor happened to seed. Seeding decides
     * that position to within a chunk, so without this the whole grid shifts
     * by up to 46ms between one play of a track and the next -- every window
     * covers different samples, marginal onsets flip, and the same track is
     * quietly a different reading each time. Nothing looks wrong when it is
     * missing; two runs over one track just stop comparing like with like.
     *
     * A chunk's stamp is in milliseconds and nothing finer is offered, so
     * the frame it recovers is within half a millisecond of the true one --
     * 22 samples at 44.1kHz. That residual cannot be removed at this
     * resolution, and it is a ninetieth of the chunk it replaces. */
    if (align_pending)
    {
        unsigned long frame;
        unsigned int skip;
        ...
        if (skip >= (unsigned int)peek->frames)
        {
            base_frames += peek->frames;
            return;
        }
...
void beat_analyse_start(void)
{
    ...
    stat_reseeds = 0;
    stat_lead_ms = 0;
    samplerate = pcmbuf_get_frequency();
...
void beat_analyse_poll(void)
{
    ...
    /* A rate change invalidates every Goertzel coefficient and every frame
     * count derived from the old rate. */
    sampr = pcmbuf_get_frequency();
    if (sampr != samplerate)
    {
        samplerate = sampr;
        beat_resync();
...
void beat_analyse_status(struct beat_status *status)
{
     ...
     * is compared against. */
    status->play_ms = 0;
    if (history_count > 0 && history[history_head].time_ms > stat_lead_ms)
        status->play_ms = history[history_head].time_ms - stat_lead_ms;
    status->samplerate   = samplerate;
    status->windows      = stat_windows;
    status->reseeds      = stat_reseeds;

    for (g = 0; g < BEAT_GROUPS; g++)
    {
        status->flux[g]      = flux_1[g];
        status->threshold[g] = beat_threshold(g);
        status->onsets[g]    = onset_seen[g];
        status->rate10[g]    = beat_rate10(g, hop_ms);
    }
}
