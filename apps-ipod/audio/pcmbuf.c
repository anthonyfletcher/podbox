...
static void pcmbuf_pcm_callback(const void **start, size_t *size)
{
    ...
    if (desc)
    {
        ...
        /* The track change above may have decided to pause -- single mode.
         * Hand back nothing rather than the next track's first chunk, which
         * would otherwise be audible before the pause takes effect. */
