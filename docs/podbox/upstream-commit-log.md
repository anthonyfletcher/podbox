...
| 2026-08-15 | `30a5f1d858` | quickscreen: redraw only the relevant viewports | **Declined** | A 132-line restructure of upstream's `quickscreen.c` so a changed setting repaints one viewport instead of the screen. It applies -- `screens/playback/quick_screen.c` still redraws whole -- but buys less here, because the shipped themes all draw the quickscreen themselves and that path returns before any of this. Wanted only as the thing `9cd4352256` below is written against, and that row is served instead by the redraw this file already does on every setting change. Settled 2026-08-31: a 132-line restructure of a file this fork has diverged in, for a path every shipped theme returns from before reaching. |
